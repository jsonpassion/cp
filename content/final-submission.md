...
| --- | --- | --- |
| **Conductor** (플래너) | `pbcopy < ~/Documents/Developer/jxc-selfeval/submissions/card_conductor_v6.3.txt` | `pbcopy < ~/Documents/Developer/jxc-selfeval/submissions/desc_conductor_v6.txt` |
...
| 시스템 프롬프트 | v6.3 (3,393자) | v6.0 | **GUI 한도 4,096자 — 초과분은 잘림**(`check_squad.py`가 ≥4096 경고) |
...
| # | 무엇을 | 명령 / 행동 | 통과 기준 |
| --- | --- | --- | --- |
| 1 | GUI 1문항 완주 (generic) | `pbcopy < ~/Documents/Developer/jxc-selfeval/gui_generic_v63.txt` → 스쿼드 대시보드 요청창에 붙여넣기(자동 승인 ON) | **태스크 0** · Conductor 메시지 = `Answer: <LETTER>` 한 줄 · 경고 **"The planner replied but produced no tasks … Each agent was given the whole request instead" = 정상**(의도된 fan-out) · Solver도 같은 답. v6.3 실측: 양쪽 `Answer: B`, 3,381 tok · 38s |
| 2 | (선택) math · coding 완주 | `gui_math_v63.txt` · `gui_coding_v63.txt` | 동일 — 태스크 0, 답만 |
| 3 | 파일 판독 | `python3 ~/Documents/Developer/jxc-selfeval/check_squad.py` | `판정: ✅ v6.3 DIRECT … 붙여넣기 가능` · 플래너 Conductor · 에이전트 2 · 모델 gpt-oss-120b ×2 · 저장 시각이 방금 |
...
| 6 | 폼 ② One-shot — coding | `pbcopy < ~/Documents/Developer/jxc-selfeval/submissions/oneshot_coding_v3.3.txt` | `{{TASK}}` 리터럴 1회, ≤32 KB |
...
| 8/23 03:1x | **v6.3 DIRECT** (분해 금지 + 자기일관성) | **v3.3** | ⏳ 채점 대기 | 뷰어 run-009 = LATEST |
