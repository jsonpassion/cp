...
### FuriosaAI 운영 가이드 (Joonwon Lee, Discord 공식)

1. 컨텍스트(입력+출력) 초과 시 **오류가 발생**할 수 있음 → *"입력을 잘라서 여러 번에 걸쳐 받도록 처리하는 에이전트를 만들어보라"* 는 공식 권장.
   → 우리 해석: **파이프라인 최전방(Planner)을 128K인 gpt-oss로** 두면 대형 입력을 통째로 수용한 뒤, 소형 컨텍스트 모델에는 요약·분할본만 넘길 수 있음 — 청킹 에이전트의 실전형.
2. **빠른 응답·배치 처리는 gpt-oss-120b와 Qwen3-32B 위주로** 사용 권장 — K-EXAONE 236B(MoE)는 상대적으로 느림. 벤치마크는 수백 문항 배치이므로 **wall-clock 캡(per-item)까지 고려하면 EXAONE 의존은 위험**.
...
## 🚨 리더보드 공개로 판명된 히든 세트의 실체 (2026-08-22 오후)

첫 런이 게시되며 리더보드 페이지가 공개한 사실 (✅ 8/22 확인: 그 행은 **심사위원 측 테스트 런**, 점수 9.3%는 무의미 — 참고용. **리더보드 본 공개는 추후**, 그 전까지 타 팀 점수는 볼 수 없음):

| 항목 | 연습(visible) | **히든(실채점)** |
| --- | --- | --- |
| math | MATH-500 L5 + AIME 2024 | **AIME 2026 + HMMT Feb 2026** (MathArena, 오염 불가) — 훨씬 어려움 |
| generic | MMLU-Pro | MMLU-Pro + **GPQA** (대학원 수준, 문항 비공개) |
| coding | SWE-bench + LCB | 동일 계열 |

- **점수 산식**: 트랙별 정확도의 **가중 평균**(0~1). 가중치는 각 run 상세에 표기 — 가중치가 균등이면 "generic 문항 수 지배" 결론이 수정됨 (1차 제출 결과에서 확인)
- **현재 캡: "no wall-clock cap, no token cap"** — 토큰 캡 실격 리스크는 현 설정에선 없음(변경 가능성 유의). 동점 시 토큰→시간 타이브레이크
- 채점은 전부 프로그램 판정(모델 심판 없음)
- 심사위원 테스트 런 프로파일(참고): **74문항 · 실행 50분(큐 포함 90분) · 274만 토큰(문항당 ~37K) · 점수 0.093** — 러너에서도 스쿼드 토큰이 무거움이 정상. 점수 자체는 의미 없음
- Check 경고로 공식 확정: `settingsOverrides`(maxTokens 등)는 **평가에 전달되지 않음**, 평가는 **tool-less**(enabledTools 자동 무력화)
