...
**우리 결정: 8개 전부 인프로세스.** 평가는 tool-less·주최측 인프라 실행이라 컨테이너의 가치(격리 도구 실행)가 쓰일 자리가 없고, 설정 비용·실패 모드만 늘어남. 워크스페이스 단계의 "그룹 네임스페이스"는 컨테이너 모드 전용 필드 → 비워둠. 설정 위치: 에이전트 카드 → 실행 모드 드롭다운(컨테이너 선택 시 이미지·타임아웃·마운트 항목 노출).
