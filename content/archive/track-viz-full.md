...
### ✅ 뷰어 v3.2 (8/22 20:55) — 간단 모드 + 투어 + 전수 측정 숫자

- **기본 진입 = 간단 모드(스코어보드)**: 헤드라인 → 카드 3장(정확도 79%→81% · 토큰 617→1,611 · 과정 = Mermaid풍 6노드 플로우 + 이벤트 시각을 따라가는 펄스) → "관측할 수 있으면 줄일 수 있다" + CTA. 용어는 괄호 설명, 본문 16px+, 강조색 2~3개
- **안내 투어 5단계**(첫 로드 자동, `T` 재시작, `D` 엑스포 자동 데모 루프) → 마지막 단계 CTA "원장 모드로 6축 보기"(`E`)
- 모든 숫자는 BENCH 상수(전수 측정: generic 79.1→80.9 · math 79.8→82.6 · LCB 75→88.9, 출력 토큰 −16/−21/−15%)에서 렌더 — 구식 수치(60%, +21%p, 100%) 잔존 0건 확인. 각주에 "20문항 표본 60%는 불운, 전수 79.1%로 교체" 정직 줄
- 실브라우저에서만 나던 `window.top` 충돌 버그 수정 → 콘솔 오류 0 · `node test.js` 통과
...
- **공개 배포 완료**: https://jsonpassion.github.io/bibimbap/ (리포 https://github.com/jsonpassion/bibimbap). 남은 것: kids.html 실전 재생 완성 후 재푸시, 발표용 스크린샷 세트, 제출 런 로그가 오면 드롭해 "실평가 리플레이"로 교체
