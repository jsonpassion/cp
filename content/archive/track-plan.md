...
## ⏱ 큐·런타임 현실 (2026-08-22 17:35 갱신)

> 🟢 **17:35 제출 폼 상태: "Nothing of yours is waiting or in flight" — 대기 0/1 · 실행 0/1.** 1차 런은 종료됨 → **Runs 탭에서 결과 판독**(점수·트랙 가중치·채점 N of M·모델별 토큰·답 추출 여부)이 지금 가장 급한 입력. 슬롯이 비어 있으므로 **2차 제출은 v3.4 저장 → 1문항 완주 검증(analyze_run.py) → check_squad.py ✅ 즉시** — 21시를 기다릴 이유가 없고, 일찍 넣을수록 3차 기회가 생긴다.
>
> 🟠 **18:44 2차 제출(v3.5) 큐 진입.** 운영진 18:34: "큐는 몰아서 처리될 예정" → 러너는 **배치 처리**, 결과 시점 예측 불가. 3차(최종, v3.6 답만 출력)는 2차 결과를 기다리지 말고 **로컬 검증(generic·LCB 완주 + quick.sh) 통과 즉시** 큐에 넣는다 — 대기 1개 제한이라 2차가 실행으로 넘어가야 가능. 01:00 마지노선.
>
> 🟢 **19:35 3차(최종) 제출 진행 — v3.7b 세트** (Qwen3 `/no_think` 플래너 + 답만 출력 솔버 + one-shot v2.2). 이후 작업은 시각화 v3 구현·피치덱·리포 정리로 전환. 큐가 배치 처리라 점수는 밤 사이 확인.
>
> (이전 15:25 기록)

- 1차 제출 런이 **1시간+ 경과에도 미완료**, 리더보드에는 6.5시간째 새 런 없음(주최측 test 런 1건뿐) → 러너는 **전역 직렬 큐**일 가능성, 런 1회 = 수 시간 단위로 가정
- 주최측 test 런 실측: 큐 진입→완료 **90분**(실행 50분 + 대기·준비 40분), 74문항. 우리 런은 플래너 중복(3~5배)만큼 더 김
- **결론: 남은 18시간 동안 제출 기회는 실질 2회.** 시행착오형 제출 불가 — 로컬에서 수렴시킨 확정본만 낸다
- 리더보드는 **추후 공개**(현재 보이는 1건은 심사위원 테스트) → 경쟁 상황을 모른 채 제출하므로 **자체 기준(베이스라인 대비 개선·문항당 토큰)으로만** 의사결정
- 스쿼드 수정은 **AI:GO GUI로만** 가능(.squad.json 수동 편집 금지) — 변경 1회당 GUI 저장 → 재검증 루프 필요, 변경 횟수를 최소화

### 수정 타임라인 (마감 8/23 12:00 KST)

| 시각 (KST) | 행동 |
| --- | --- |
| ~17:00 | 플래너 모델 A/B 완료 → Conductor 모델 확정, published requests로 프롬프트 봉인 |
| 1차 런 완료 즉시 | **우리 Run 상세**(로그인 영역)에서 가중치·점수·토큰 확인 → 린/앙상블 결정. ⚠️ 리더보드는 추후 공개라 **타 팀 대비 위치는 알 수 없음** — 절대 점수와 로컬 selfeval만으로 판단 |
| **~21:00** | **2차 제출 = 최종 후보본**(중복 제거 + 확정 프롬프트). 이게 사실상 본선 제출 |
| 21:00~01:00 | 시각화 뷰어·피치덱·리포 정리 (제출과 병렬) |
| **01:00 하드 데드라인** | 3차(최종) 제출 큐 진입 마지노선 — 이후 큐 정체 시 미채점 위험 |
| 08:00~11:30 | 플랫폼 제출물(피치덱 PDF·요약·리포) 마감 작업 |
